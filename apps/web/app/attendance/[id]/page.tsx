...
export default async function SessionAttendancePage({
  ...
  try {
    ...
    // Merge review queue data with attendance data for names and IDs
    ...
    return (
      <div className="page-shell">
        <div className="page-inner">
          ...
          {/* Status Summary */}
          <div className="grid grid-cols-2 sm:grid-cols-5 mb-12 border-t" style={{ borderColor: 'var(--border)' }}>
            <div className="px-4 py-5 border-b border-r first:border-l">
              <div className="text-3xl font-mono tabular-nums font-semibold" style={{ color: 'var(--accent-good)' }}>{counts.present}</div>
              <div className="text-xs uppercase tracking-wide mt-2" style={{ color: 'var(--muted)' }}>Present</div>
            </div>
            <div className="px-4 py-5 border-b border-r">
              <div className="text-3xl font-mono tabular-nums font-semibold" style={{ color: 'var(--accent-bad)' }}>{counts.absent}</div>
              <div className="text-xs uppercase tracking-wide mt-2" style={{ color: 'var(--muted)' }}>Absent</div>
            </div>
            <div className="px-4 py-5 border-b border-r">
              <div className="text-3xl font-mono tabular-nums font-semibold" style={{ color: 'var(--accent-warn)' }}>{counts.left_early}</div>
              <div className="text-xs uppercase tracking-wide mt-2" style={{ color: 'var(--muted)' }}>Left Early</div>
            </div>
            <div className="px-4 py-5 border-b border-r">
              <div className="text-3xl font-mono tabular-nums font-semibold" style={{ color: counts.review > 0 ? 'var(--accent-warn)' : 'var(--foreground)' }}>{counts.review}</div>
              <div className="text-xs uppercase tracking-wide mt-2" style={{ color: 'var(--muted)' }}>Needs Review</div>
            </div>
            <div className="px-4 py-5 border-b border-r">
              <div className="text-3xl font-mono tabular-nums font-semibold text-white">{counts.exceptions}</div>
              <div className="text-xs uppercase tracking-wide mt-2" style={{ color: 'var(--muted)' }}>Exceptions</div>
            </div>
          </div>
                ...
                {enhancedReviewQueue.map((item: any) => (
                  <div key={item.id} className="pt-4 pb-6 border-b" style={{ borderColor: 'var(--border)' }}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <form className="bg-black/20 p-3 rounded-md border" style={{ borderColor: 'var(--border)' }} action={resolveReviewItem.bind(null, item.id, id)}>
                        <div className="text-xs uppercase tracking-wide mb-2 text-white/70">Resolve Override</div>
                        <div className="flex gap-2 items-center">
                          <select name="new_status" required className="field-input py-1 text-sm bg-transparent !w-32">
                            <option value="">Status...</option>
                            <option value="present">Present</option>
                            <option value="absent">Absent</option>
                            <option value="left_early">Left Early</option>
                          </select>
                          <input type="text" name="notes" placeholder="Notes (optional)" className="field-input py-1 text-sm bg-transparent !w-40" />
                    ...
                    {item.evidence && item.evidence.length > 0 && (
                      <div className="mt-4">
                        <div className="text-xs uppercase tracking-wide mb-2" style={{ color: 'var(--muted)' }}>Evidence Photos</div>
                        <div className="flex gap-2 overflow-x-auto pb-2">
                          {item.evidence.map((ev: any, idx: number) => (
                            <div key={idx} className="relative group">
                              ...
                              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center text-xs text-white transition-opacity">
              ...
              {safeAttendance.map((item: any) => (
                <div key={item.id} className="ledger-row" style={{ gridTemplateColumns: '2fr 1fr 1fr 1.5fr' }}>
                  <div>
                    ...
                    <div className="text-xs" style={{ color: 'var(--muted)' }}>{item.students.roll_number || '—'}</div>
                  </div>
                  ...
                  <div className="text-sm">
                    {item.exception_applied ? (
                      <span className="badge badge-neutral bg-white/5 py-0.5" style={{ color: 'var(--accent-good)' }}>Applied</span>
                    ) : '—'}
                  </div>
                </div>
              ))}
    ...
    return (
      <div className="page-shell">
        <div className="page-inner">
          ...
          <p className="text-sm mt-4 text-red-400">{error.message}</p>
        </div>
      </div>
    )
  }
