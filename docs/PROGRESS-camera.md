...
- Formal eval harness (`eval_calibration.py`) now correctly reuses production `find_best_match` matching logic (not reimplemented math) and was run once against the 9-subject `test-images` dataset: 6 genuine comparisons (0% false reject rate), 99 imposter comparisons (4.04% false accept rate) at current `match_threshold=0.5` (raw data in `eval_results.csv`).
**Files changed:**
