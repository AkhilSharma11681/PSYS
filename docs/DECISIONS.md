...
- **Resolved known issue (pagination):** the cross-student duplicate-face query in enrollment-worker (`fetch_other_biometrics()` in `worker.py`) was previously noted as lacking pagination. Investigation confirmed `.range()` batched pagination was already implemented in commit `05bd5ed` and is now verified via mocked unit tests in `test_duplicate_check.py` (see 2026-09-12 decision entry).
...
- **Known issue:** `resolveReviewItem()` in `apps/web/lib/enrollment/attendance.ts` does not verify affected row count after updating `final_attendance`. When called by a user unauthorized by RLS (e.g. a teacher not assigned to the session's class), Supabase silently updates zero rows without erroring, causing the function to report success and record an `audit_logs` entry even though no status changed. Not fixed yet; needs an affected-row count check (e.g. via `.select()` on update) to reject unauthorized/zero-row updates.
...
- **Permitted-exit investigation & partial verification (2026-09-12).**
  - **CONFIRMED:** Permitted-exit feature is fully built across the entire stack (`/sessions/[id]` UI form, `markPermittedExit()` server action in `apps/web/lib/enrollment/exceptions.ts`, `session_exceptions` table, `gap_check.py` excusal logic, and `orchestrator.py` finalization integration) — not a UI stub.
  - **CONFIRMED:** `markPermittedExit()` works correctly via its real code path — successfully created a `session_exceptions` row when called through an authenticated test route mimicking the real UI flow.
  - **CONFIRMED (Design Note, Not a Bug):** `markPermittedExit()` only supports real-time exits (`exit_at = now()`) without historical backdating. This matches the real UI (no time picker, just a "Mark Exit" button) and is treated as intentional design.
  - **NOT YET CONFIRMED:** Whether `gap_check.py`'s excusal math actually produces the correct `final_attendance` outcome (`status`, `exception_applied = true`) end-to-end. Multiple attempts to construct a synthetic test (inserting `attendance_observations` rows to create a controlled gap) were blocked by: (1) needing to null/restore `finalized_at` to re-trigger finalization, (2) a column name mismatch in the test script (`confidence_score` vs actual `similarity_score`), and (3) an unexpected RLS policy violation when inserting into `attendance_observations` using the service-role key, which should normally bypass RLS — this RLS finding itself may be worth investigating separately, flagged as an open question.
  - **Test Cleanup:** All test artifacts (`session_exceptions` rows, `finalized_at` modifications) were created and cleaned up correctly each time — confirmed via real re-queries, leaving 0 stray rows in the database.
  - **Recommendation:** A proper end-to-end excusal test remains a valid open item for a future session, ideally either fixing the service-role RLS issue first, or testing via a real live camera session with an actual gap rather than synthetic data.

- **Duplicate-face detection dimension guard regression fix & pagination test verification (2026-09-12).**
  - **The Regression:** The dimension-mismatch guard in `is_duplicate_face()` (`services/enrollment-worker/app/worker.py`) was accidentally removed in commit `05bd5ed` during an unrelated refactor adding InsightFace embeddings and pagination.
  - **The Risk:** Any institution with mixed dlib (128-D) and InsightFace (512-D) embeddings in `student_biometrics` would crash the enrollment worker with an unhandled `ValueError` in `math.dist()` instead of gracefully skipping incompatible comparisons.
  - **The Fix:** Restored the dimension check (`len(new_embedding) != len(existing_embedding)`) before computing Euclidean distance in `is_duplicate_face()`.
  - **Verification:** Verified all 9 unit test cases pass in `services/enrollment-worker/test_duplicate_check.py` (7 original matching cases including dimension mismatch handling + 2 pagination mock cases).
  - **Pagination Note:** Confirmed `fetch_other_biometrics()` pagination logic was already correctly implemented in `worker.py` (resolving the earlier stale "known issue" note) and now has proper mocked test coverage (2 new cases added simulating multi-page and single-page PostgREST responses).
- **InsightFace formal threshold calibration — corrected group-photo evaluation (2026-09-12).**
  - **Context & Root Cause of Initial Flawed Eval:** The initial execution of the formal evaluation script (`eval_calibration.py`) reported a 4.04% False Accept Rate (FAR) across 99 imposter comparisons and 6 genuine comparisons. Investigation revealed this was caused by a ground-truth labeling flaw: group photos (`WhatsApp Image...`) were labeled by filename prefix (`'whatsapp'`), causing different faces of the same subjects appearing across group photos and solo images to be miscounted as imposter comparisons rather than genuine matches.
  - **Corrected Methodology:** Updated `eval_calibration.py` to detect individual faces in group photos, sort bounding boxes spatially from left to right (`face.left`), and map them to their known ground-truth identities (`naveen`, `akhil`, `aditya raj`, `krish garg`, `abhishek`). Krish Garg (who only appears in group photos without independent enrollment/solo photos) was properly configured as a genuine imposter against all enrolled subjects while excluding `krish garg` vs `krish garg` comparisons from genuine pairs. Pairwise evaluations strictly reused production `find_best_match` logic.
  - **Verified Results (`match_threshold = 0.5`):**
    - *Genuine Comparisons:* 24 total, 0 false rejects (FRR = 0.0000 / 0.0%). Minimum genuine similarity observed was ~0.646 (max distance ~0.354).
    - *Imposter Comparisons:* 228 total, 0 false accepts (FAR = 0.0000 / 0.0%). Maximum imposter similarity observed was ~0.301 (min distance ~0.699).
  - **Calibration Assessment:** This wide separation margin (~0.345 gap between max imposter similarity 0.301 and min genuine similarity 0.646) provides the strongest empirical calibration evidence gathered so far supporting `match_threshold = 0.5` as an optimal operating point for InsightFace on this dataset.
  - **Open Limitation:** The evaluation dataset covers 7–8 unique subjects across test images, which is not yet representative of a full classroom environment (20–30+ students). This calibration should be revisited with broader multi-subject live classroom datasets before being treated as fully production-validated.
- **Formal confirmation on project memory files git tracking (2026-09-13).** Confirmed that all shared memory and documentation files (`CLAUDE.md`, `docs/ARCHITECTURE.md`, `docs/DECISIONS.md`, `docs/PROGRESS-enrollment.md`, `docs/PROGRESS-camera.md`, and `OPERATIONS.md`) must remain strictly tracked in git and not ignored. The prior open question regarding their tracking status has been resolved and closed across all progress logs.
