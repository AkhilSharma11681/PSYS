...
6. Never add a 'Co-Authored-By: Claude Code' (or similar AI attribution) trailer to any git commit message. Plain commit messages only, no exceptions, unless explicitly asked to include one in a specific task.
7. **STANDING RULE:** If a task's stated constraint blocks progress (e.g. 'read-only, no code changes', 'do not execute', 'manual verification only'), STOP immediately and report the exact blocker back to the supervisor. Do NOT write a workaround, substitute script, or alternative method to route around the constraint, even if the original approach is technically infeasible. Wait for an explicit new instruction before proceeding. This applies even under auto mode.
